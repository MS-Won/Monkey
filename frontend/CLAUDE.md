...
### ✅ 해결: `/reading` 38초의 진짜 원인은 Okt 첫 호출이었다 (2026-08-04)

**이전 진단("출력 토큰 2배가 주원인")은 틀렸다.** 구간 계측으로 다시 재보니:

| 구간 | 실측 |
|---|---|
| 프로덕션 `/health` 첫 호출 | 14.5초 (콜드스타트) |
| 프로덕션 `/reading` **첫** 호출 | **38.3초** |
| 프로덕션 `/reading` 두 번째 | **7.0초** |
| 로컬 `lexicon` 첫 호출 / 워밍 후 | 2.48초 / **0.02초** |

38초는 정상 상태가 아니라 깨어난 뒤 **첫 호출에만** 났다. `Okt()` 생성만으로는 JVM이
준비되지 않고 첫 형태소 분석에서 클래스 로딩·JIT가 일어나는데, 개발 PC 2.5초가
Render 무료(0.1 CPU)에서 30초 안팎으로 불어난 것이다. `/health`는 Flask만 확인해
워밍 역할을 못 했다(그래서 헬스체크 통과 후에도 첫 사용자가 이 비용을 물었다).

- **해결**: `keyword_server.py`의 `warm_up_okt()`를 모듈 임포트 시점에 호출한다.
  gunicorn이 워커를 띄우며 임포트하므로 헬스체크 무렵엔 워밍업이 끝나 있다.
  `/health`가 `oktReady`를 함께 반환한다.
- **추가**: `/reading`을 OpenAI 2회 병렬(`summary`+`oneLine` / `categories`)로 쪼갰다.
  로컬 median 6.4초 → 5.3초, 같은 시간에 카테고리가 2개→3개로 늘었다(출력 500→720토큰).
  비용 $0.00050 → $0.00081(입력 프롬프트가 두 번 들어간다).
- 응답에 `timings: {lexicon, openai, total}`이 실려 온다. 회귀하면 여기부터 본다.
- 여전히 남은 것: 앱에 요청 타임아웃과 재시도가 없다. Render 무료 플랜 콜드스타트 중에
  HTML 오류 페이지를 받으면 `해몽을 가져오지 못했습니다`가 그대로 뜬다
  → **Render starter 플랜 적용(사용자 작업 2번)이 유일한 해법**.

### 이번 세션(08-04) 완료 — 카드 글자를 그림 밖으로

카드 한글 이름/뜻이 일러스트 한복판에 구워져 있어(`scripts/apply-card-labels.py`가
y=244에 합성) 글자와 그림이 서로 간섭했다. 예: 02 Cocoon의 `변화·전환·성장통`이
요정의 화관·날개 위에 얹혀 있었다.

- 카드 PNG 30장을 **`img/NN_Name.png` 원본 그대로** 교체(1024×1536, 2:3 불변이라 드롭인).
  원본 하단의 영문 이름판은 자체 카투슈 안에 있어 그림과 안 겹친다.
- `apply-card-labels.py` 삭제 → **`scripts/sync-card-art.py`**(복사만 한다)로 대체.
  한글 이름/뜻의 단일 소스는 `archetypeCards.ts`의 `nameKo`/`meaning` 하나로 정리됐다.
- 신규 `components/DreamCard/CardLabel.tsx` — 카드 **바깥 위쪽**에 이름/뜻을 그린다.
  ResultScreen·DiaryDetailScreen에서 사용.
- FanCarousel은 카드가 부채꼴로 겹쳐 있어 카드마다 라벨을 못 붙인다. 대신
  DiaryScreen 헤더에 포커스 카드의 `이름 · 뜻`을 한 줄 표시한다.
  **`FanCarousel.tsx`는 수정하지 않았다.**
- `DreamCard.tsx`에서 옛 오버레이 설계의 죽은 코드 정리(`Scrim`·`pad2`·미사용 스타일 9종).

### ▶ 다음 세션에서 바로 할 일

**사용자 수동 작업 2건이 유일한 잔여 항목이다** (아래 "사용자만 할 수 있는 일" 참고).
카드 앞면이 바뀌었으므로 **스토어 스크린샷 `5-card`·`6-interpretation` 재캡처 필요**.
